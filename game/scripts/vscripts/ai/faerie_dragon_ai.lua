...
function thisEntity:AIThink()
  ...
  if FindAggro(self) then
    if self:UseHeal() then return 1.0 end
    AttackTarget(self)
    return .3
  end
...
function thisEntity:UseHeal()
  local ability = self:FindAbilityByName("faerie_dragon_heal")
  ...
  if castRange then
    ...
    target = FindFirstUnit(targets, function(target) 
      return not IsCustomBuilding(target) and target:GetHealth() < target:GetMaxHealth() - 50
    end)
  end
